  ...
  <main class="flex-1 flex flex-col items-center justify-center p-4 my-8">
    ...
    <div class="w-full max-w-md bg-[#0a0f1d]/95 border border-slate-800/80 rounded-2xl p-6 sm:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.65)] backdrop-blur-xl relative">
      ...
      <div class="w-full flex flex-col items-center mb-1">
        <div id="googleBtnContainer" class="w-full flex justify-center min-h-[40px]">
          <button type="button" onclick="triggerGoogleSignIn()" class="w-full bg-[#0c1220] hover:bg-[#131d33] active:scale-[0.985] text-slate-200 font-semibold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-3 border border-slate-800 hover:border-slate-700 transition duration-150 shadow-sm cursor-pointer">
      ...
      <!-- Separador Visual -->
      <div class="flex items-center w-full gap-3 my-3.5 select-none">
        <div class="flex-1 h-px bg-slate-800/80"></div>
        <span class="text-[10px] text-slate-500 font-mono tracking-wider uppercase">o con credenciales</span>
        <div class="flex-1 h-px bg-slate-800/80"></div>
      </div>
  ...
  <script>
    ...
    async function handleGoogleCredentialResponse(response) {
      ...
      if (alertBox && alertText) {
        ...
        alertText.textContent = "Verificando credencial institucional de Google...";
      ...
      try {
        ...
        if (data && data.success) {
          ...
          if (alertBox && alertText) {
            ...
            alertText.textContent = data.message || "Error al autenticar con Google.";
            alertBox.classList.remove('hidden');
          }
        }
      } catch (err) {
    ...
    function submitAuthForm(e) {
      ...
      const payload = isRegisterMode 
        ? { correo: email, contrasena: pass, password: pass, nombre } 
        : { correo: email, contrasena: pass, password: pass };
